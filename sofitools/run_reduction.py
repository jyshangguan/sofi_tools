...
def find_imflat_raw(file_path):
    '''
    Find imaging flat files.
    '''
    if not os.path.isabs(file_path):
        file_path = '{0}/{1}'.format(cwd, file_path)
    fList = sorted(glob('{}/*.fits'.format(file_path)))
...
def find_imscience_raw(file_path):
    '''
    Find imaging science files.
    '''
    if not os.path.isabs(file_path):
        file_path = '{0}/{1}'.format(cwd, file_path)
    fList = sorted(glob('{}/*.fits'.format(file_path)))
...
def write_imflat_sof(file_list, work_path, reduced_dir='reduced'):
    '''
    Write the SOF file of flat data.
    '''
    if not os.path.isabs(work_path):
    ...
    sof_path = '{0}/{1}/imflat.sof'.format(work_path, reduced_dir)
    with open(sof_path, 'w') as f:
        for fn in file_list:
            f.write('{} IM_FLAT\n'.format(fn))
    return sof_path
...
def write_imjitter_sof(file_list, flat_list, work_path,
                     reduced_dir='reduced'):
    '''
    Write the SOF file of imaging science data.
    '''
    if not os.path.isabs(work_path):
        work_path = '{0}/{1}'.format(cwd, work_path)
    ...
    filename = file_list[0].split('/')[-1][:-5]
    sof_path = '{0}/{1}/{2}_imjitter.sof'.format(work_path, reduced_dir, filename)

    with open(sof_path, 'w') as f:
        for fn in file_list:
            f.write('{} IM_JITTEROBJ\n'.format(fn))
        for flat in flat_list:
            f.write('{} MASTER_IMG_FLAT\n'.format(flat))
    return sof_path
